...
public class TextFileManager {
    ...
    public TextFileManager(String filePath){
        textFile = new File(filePath); // creates a "label" for the file

        File parentDir = textFile.getParentFile(); // Gets the folder containing the file
        if (parentDir != null && !parentDir.exists()){ // checks if folder exists
            parentDir.mkdirs(); // Create the folder
            System.out.println("[TEXT] Created directory: " + parentDir.getAbsolutePath());
        }

        try {
            if (!textFile.exists()){ // checks if file exists
                textFile.createNewFile(); // create an empty file
    ...
    public void displayFileProperties(){
        ...
        System.out.println("Exists: " + textFile.canRead());
        System.out.println("Readable: " + textFile.canWrite());
